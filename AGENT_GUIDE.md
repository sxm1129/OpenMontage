...
**"No reusable creative components" is about visual sameness, not about how text gets drawn.** It does not mean "skip real text rendering." Any verbatim on-screen text (brand name, slogan, CTA, end-card copy — especially CJK or any non-Latin script) must still be composited as real text via a real font (Remotion DOM text with the bundled CJK fallback in `remotion-composer/src/fonts.ts`, or ffmpeg `drawtext` with a real font file if the runtime is ffmpeg-only) — bespoke styling and motion are welcome, bespoke *glyphs* are not. **Never ask an image-generation model to paint the words into the picture.** Diffusion models routinely distort embedded text, and for CJK the failure mode is a plausible-looking but wrong character substitution that's easy to miss in a quick review, not an obvious tofu box. A "text legibility unverified" self-review finding on a generated image is a **critical** blocker, not a suggestion to note and move on from. See `skills/pipelines/cinematic/asset-director.md` § "Never Bake Verbatim Text Into a Generated Image" for the full rationale.
...
**Multi-shot character/product consistency is not free — it requires a reference image, not just repeated prompt wording.** Generating 3-4 shots of "the same" character or product from independent text-only prompts, even with identical descriptive phrases, routinely produces a visibly different design per shot (this has actually happened in production: a robot-vacuum brand video with four generated clips showed four different-looking vacuums). If a brief needs the same character/product across multiple generated video shots:
- Generate one hero reference image first, then check whether the chosen video model actually supports `image_to_video`/`reference_to_video` conditioning — not all do (e.g. `leapfast/ltx-2.3` via `maas_video` is text-to-video only and silently ignores a reference image).
- Reuse that exact reference across every shot of that character/product. `seedance_video` (Seedance 2.0) has the strongest documented support for this — see `.agents/skills/seedance-2-0/SKILL.md`'s `[reference_image: ...]` + `[identity_lock]` syntax and identity-anchor phrases.
- If the approved/available model has no reference support, tell the user before generating rather than letting per-shot visual drift surface only in the final render.
...
`text_card`, `stat_card`, `callout`, `comparison`, `hero_title`, `terminal_scene`, `anime_scene`, `screenshot_scene`, `bar_chart`, `line_chart`, `pie_chart`, `kpi_grid`, `progress_bar`. Overlay types include `section_title`, `stat_reveal`, `hero_title`, `provider_chip`.
