...
- **Bake** `bake-basemap.mjs` (puppeteer + MapLibre; env-driven: `NAME STYLE COUNTRIES CENTER ZSTART ZEND PITCH BEARING FPS DUR`): drive the camera **zoom→hold** (`easeInOutCubic`) and **`await map.once('idle')` before every frame** so each frame has complete tiles (Remotion `delayRender` technique → no tile pop). The helper resolves Chrome itself, derives the overseas-territory filter per subject, and **fails closed** on anything that would degrade the asset — see below. `preserveDrawingBuffer:true`, `fadeDuration:0`.
- **Deps — the libs are vendored, the tiles are not.** maplibre / topojson / world-atlas are read from `vendor/maps/` (pinned + hashed in `vendor/THIRD_PARTY_NOTICES.md`); the bake makes **no library network call** and will not start if a vendored file is missing. **Never reintroduce a CDN `<script>` or `fetch` here** — that is what previously hung the bake forever on a jsdelivr blip (the world-atlas `fetch` had no `.catch()`). The **Esri/CARTO tile endpoints cannot be vendored** (real imagery is the point; ToS-bound, unbounded) and remain a live dependency — re-verify availability + ToS on any version bump.
- **Fail-closed contract (do not soften any of these to a warning).** Non-zero exit + **no MP4 and no `coords.json`** (suspect frames are left for inspection) when: **any tile errors** (zero tolerance), a frame misses the idle timeout, a requested country isn't in world-atlas, `HOLD > 1` (no hold frame → borders never projected), or a vendored lib is missing/corrupt. `tests/skills/test_bake_basemap_fails_closed.py` locks this in.
  - **Why a tile-error count and not just the idle check:** MapLibre's `areTilesLoaded()` counts an **errored** tile as *loaded*, so `idle` fires perfectly happily when every tile 404s. The bake used to emit a 1920x1080 MP4 of solid `#05070d`, exit **0**, and log "all N frames reached map idle (complete tiles)" — a silent downgrade AGENT_GUIDE.md forbids. The guard hooks `map.on('error')` instead, which is the only signal that distinguishes "loaded" from "gave up". The helper then **encodes the frames to an all-intra MP4 itself** (`ffmpeg -framerate FPS -i f%04d.png -c:v libx264 -g 1 -pix_fmt yuv420p`) and writes everything to **`$OUT` (default `cwd`)**, not the skill dir → `<NAME>.mp4` + `<NAME>-coords.json`.
...
- **Smoothness = per-frame complete tiles + eased camera.** In the bake, `await map.once('idle')` before each screenshot (= Remotion `delayRender`) and ease the camera with `easeInOutCubic` (= interpolate+Easing). `preserveDrawingBuffer:true`, `fadeDuration:0`, large `maxTileCacheSize`. **`idle` proves the camera settled, never that imagery arrived** — an errored tile counts as loaded, so idle alone happily green-lights a blank bake. Correctness of the imagery is the tile-error guard's job, not idle's.
